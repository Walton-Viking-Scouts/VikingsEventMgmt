...
Verify by running existing integration tests from Phases 2-6 that exercise cross-platform query paths:
`grep -rn "getSections\|getEvents\|getSharedEventMetadata\|getAttendance" src/ --include="*.test.*"` -- confirm test files exist covering these query functions.
Run `npm run test:run -- --reporter=verbose` to confirm all query-function tests pass with consistent return shapes.
Run `npm run lint` -- zero errors.
Run `npm run test:run` -- all tests pass.
Run `npm run build` -- build succeeds.
</verification>
