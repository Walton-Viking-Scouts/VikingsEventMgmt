...
must_haves:
  truths:
    ...
    - "Shared attendance uses the unified attendance store with an isSharedSection marker field, keyed by [eventid, scoutid] like regular attendance. This implements the CONTEXT.md locked decision ('shared attendance uses the same store as regular attendance, with a marker field') which supersedes the roadmap success criteria wording about a separate [eventid, sectionid] compound key."
