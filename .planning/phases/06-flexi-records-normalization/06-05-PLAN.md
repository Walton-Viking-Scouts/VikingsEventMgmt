...
1. Replace the localStorage scanning block that filters for `viking_flexi_structure_` keys (~line 260) with `databaseService.getFlexiStructure(recordId)` — the component should already know the recordId from context/props. If it's iterating over all structures, use `IndexedDBService.getAllFlexiRecordStructures()` via databaseService (add `getAllFlexiStructures()` method to DatabaseService if needed, or use the data already available from FlexiRecordDataService).
...
1. Understand what `dataItem` is and where it comes from. If it's from a list of stored items, the approach needs to change to use `databaseService.getFlexiData()` or `databaseService.getFlexiRecordDataByExtra()` calls instead.

2. Replace the key-string parsing with proper service calls that return structured data with explicit fields (extraid, sectionid, termid) rather than parsing them from key strings.
...
- Replace the localStorage key scanning that filters for `viking_flexi_data_${vikingMoversRecord.extraid}_${sectionId}_` (~line 82) with `databaseService.getFlexiData(vikingMoversRecord.extraid, sectionId, termId)` — determine termId from context
