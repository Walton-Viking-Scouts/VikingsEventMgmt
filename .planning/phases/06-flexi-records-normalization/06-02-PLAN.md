...
must_haves:
  truths:
    ...
    - "DatabaseService.getFlexiRecordDataByExtra returns all flexi_data rows for a given extraId"
    - "DatabaseService.getAllFlexiStructures returns all flexi structures without requiring a specific recordId"
    - "All eight methods work on both web (IndexedDB) and native (SQLite) platforms"
  artifacts:
    - path: "src/shared/services/storage/database.js"
      provides: "8 working flexi methods replacing Phase 6 stubs"
...
<objective>
Replace all 6 DatabaseService flexi stub methods with working implementations using Zod validation, IndexedDB (web), and SQLite (native), and add 2 new query methods (getAllFlexiStructures, getFlexiRecordDataByExtra) needed by Plan 05 consumers.

Purpose: Provides the platform-agnostic DatabaseService API that FlexiRecordDataService will call in Plan 03 and that Plan 05 consumers (CampGroupsView, useSignInOut, useSectionMovements) will call directly, completing the storage layer for flexi records.
Output: DatabaseService with 8 working flexi methods (5 get + 3 save) on both platforms.
</objective>
...
**getAllFlexiStructures():**
- New method (not replacing a stub — this is a new query needed by CampGroupsView in Plan 05)
- Web path: `return await IndexedDBService.getAllFlexiRecordStructures()`
- Native path: `SELECT * FROM flexi_structure`, return `res.values || []`
- Read-path resilience: return `[]` on error with Sentry capture
...
**getFlexiRecordDataByExtra(extraId):**
- New method (not replacing a stub — this is a new query needed by useSignInOut and useSectionMovements in Plan 05)
- Web path: `return await IndexedDBService.getFlexiRecordDataByExtra(extraId)`
- Native path: `SELECT * FROM flexi_data WHERE extraid = ?` with `[String(extraId)]`, return `res.values || []`
- Read-path resilience: return `[]` on error with Sentry capture
- This method discovers all flexi_data for a given extraId without needing termId upfront (the current consumer code discovers termId from key scanning)
...
Remove underscore prefixes from all parameter names. Update JSDoc to remove `@throws` annotations since methods no longer throw.
  ...
  <done>All 6 DatabaseService flexi stubs are replaced with working implementations, plus 2 new query methods (getAllFlexiStructures, getFlexiRecordDataByExtra) are added. No "Phase 6" stub errors remain.</done>
</task>
...
4. All 8 methods have Zod validation on writes, read-path resilience on reads
5. Both web (IndexedDB) and native (SQLite) code paths exist
</verification>

<success_criteria>
All 8 DatabaseService flexi methods (6 stub replacements + 2 new query methods) work on both platforms. No stub errors remain. Zod validation applied at write boundary. Read paths are resilient with fallback values.
</success_criteria>
