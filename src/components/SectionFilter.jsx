 ...
 */
function SectionFilter({ sectionFilters, onFiltersChange, sections, className = '' }) {
  ...
  const getSectionColor = (section) => {
    ...
    if (sectionType.includes('earlyyears')) return 'bg-scout-red text-white border-scout-red';
    if (sectionType.includes('beavers')) return 'bg-scout-blue text-white border-scout-blue';
    if (sectionType.includes('cubs')) return 'bg-scout-forest-green text-white border-scout-forest-green';
    if (sectionType.includes('scouts')) return 'bg-scout-navy text-white border-scout-navy';
    if (sectionType.includes('adults')) return 'bg-scout-purple text-white border-scout-purple';
    if (sectionType.includes('waitinglist')) return 'bg-scout-teal text-white border-scout-teal';
  ...
  return (
    <div className={`flex gap-2 flex-wrap items-center ${className}`} role="group" aria-label="Section filters">
      {/* Individual section filters */}
      {sections.map((section) => {
        const isActive = sectionFilters[section.sectionid];
        const activeStyles = getSectionColor(section);
        const inactiveStyles = 'bg-white text-gray-600 border border-gray-300 hover:bg-gray-50';
