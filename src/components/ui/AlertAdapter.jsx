...
const AlertAdapter = ({
  ...
  const { title, description, actions, content } = useMemo(() => {
    ...
    React.Children.forEach(children, (child) => {
      if (React.isValidElement(child)) {
        if (child.type === AlertTitle || child.type?.name === 'AlertTitle') {
          extractedTitle = typeof child.props.children === 'string' 
            ? child.props.children 
            : React.isValidElement(child.props.children) 
              ? '' 
              : String(child.props.children);
        } else if (child.type === AlertDescription || child.type?.name === 'AlertDescription') {
          extractedDescription = typeof child.props.children === 'string' 
            ? child.props.children 
            : React.isValidElement(child.props.children) 
              ? '' 
              : String(child.props.children);
          ...
          // Other content
          remainingContent += typeof child === 'string' ? child : '';
        }
  ...
  // For scout variants, we need custom icons that match the original Alert component
  const getScoutIcon = useCallback((alertVariant) => {
