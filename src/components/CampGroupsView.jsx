...
function CampGroupsView({
  ...
  useEffect(() => {
    ...
    const loadCampGroups = async () => {
      ...
      try {
        ...
        organized.summary.vikingEventDataAvailable = Boolean(
          primaryCampGroupData && Object.keys(primaryCampGroupData).length,
        );
  ...
  const handleMemberClick = (member) => {
    ...
    if (cachedMember) {
      ...
      enrichedMember = {
        ...
        sections: [
          findMemberSectionName(
            cachedMember.sectionid || cachedMember.section_id,
            sectionsCache,
          ) ||
            member.sectionname ||
            "Unknown",
        ...
        sectionname:
          findMemberSectionName(
            cachedMember.sectionid || cachedMember.section_id,
            sectionsCache,
          ) || member.sectionname, // Also set sectionname for consistency
      };
    } else {
  ...
  const updateGroupsOptimistically = useCallback(
    (moveData) => {
      setOrganizedGroups((prevGroups) => {
        ...
        const targetYoungPeople = Array.isArray(targetGroup.youngPeople)
          ? targetGroup.youngPeople
          : [];
        if (
  ...
  const updateDemoCampGroupAssignment = useCallback(
    (member, newGroupNumber, memberSectionId) => {
      ...
      const memberIndex = cached.items.findIndex(
        (m) => Number(m.scoutid) === memberScoutId,
      );
  ...
  const handleMemberMove = useCallback(
    async (moveData) => {
      ...
      const memberSectionType = findMemberSectionType(
        memberSectionId,
        sectionsCache,
      );
  ...
  const handleGroupRename = useCallback(
      ...
      try {
        ...
        logger.info(
          "Starting group rename operation",
          {
            ...
            totalMembers: Object.values(membersBySection).reduce(
        ...
        for (const [sectionId, members] of Object.entries(membersBySection)) {
          ...
          try {
            const scoutIds = Array.from(
              new Set(members.map((member) => String(member.scoutid))),
            );
            if (!/^f_\d+$/.test(String(flexiRecordContext.columnid))) {
              throw new Error(
                `Invalid FlexiRecord field ID '${flexiRecordContext.columnid}'`,
              );
            }
    ...
    [flexiRecordContext, showToast, events, setOrganizedGroups],
  );
          ...
          try {
            ...
            for (const member of members) {
              try {
                ...
                if (!memberSectionType) {
                  throw new Error(
                    `Cannot find section type for member ${member.scoutid} in section ${memberSectionId}`,
                  );
                }
            ...
            if (sectionSuccessCount === members.length) {
              ...
              throw new Error(
                `Partial success: ${sectionSuccessCount}/${members.length} members moved to unassigned`,
              );
            } else {
              throw new Error(
                `No members successfully moved to unassigned (${sectionFailureCount} failures)`,
              );
            }
          } catch (sectionError) {
            failedUpdates++;
        ...
        if (successfulUpdates > 0 && failedUpdates === 0) {
          ...
          setOrganizedGroups((prevGroups) => {
            ...
            if (deletedGroup) {
              ...
              const allMembers = [
                ...(deletedGroup.leaders || []),
                ...(deletedGroup.youngPeople || []),
              ];
  ...
  if (loading) {
    return (
      <LoadingScreen message="Loading camp groups..." data-oid="nkou28a" />
    );
  }

  if (error) {
    return (
      <Alert variant="danger" className="m-4" data-oid="m:nbphh">
        <Alert.Title data-oid="8skxgs8">Error Loading Camp Groups</Alert.Title>
        <Alert.Description data-oid="6:ch5hh">{error}</Alert.Description>
        <Alert.Actions data-oid="96f8pkn">
          <Button
            variant="scout-blue"
  ...
  return (
    <div className="camp-groups-view" data-oid="akswcak">
      {/* Header with summary stats */}
      <div className="mb-6" data-oid="hzg:2mf">
        <div
          className="flex flex-wrap gap-4 mb-4 items-center justify-between"
          data-oid="tn_ckjp"
        >
          <div className="flex flex-wrap gap-4" data-oid="md231fc">
            <Badge variant="scout-blue" size="md" data-oid="83j85dp">
              {summary.totalGroups || 0} Groups
            </Badge>
            <Badge variant="scout-green" size="md" data-oid="08oh1cx">
        ...
        {!summary.vikingEventDataAvailable && (
          <Alert variant="warning" className="mb-4" data-oid="pev-9h8">
            <Alert.Title data-oid="19b_bvk">
              No Viking Event Management Data
            </Alert.Title>
            <Alert.Description data-oid="aiofg.1">
              ...
              {!flexiRecordContext && (
                <>
                  <br data-oid="1u6baf7" />
                  <strong data-oid="8w:p371">Note:</strong> Drag and drop
      ...
      {filteredAndSortedGroups.length === 0 ? (
        <div className="text-center py-12" data-oid="61nx20.">
          <svg
          ...
          <p className="text-gray-600" data-oid="_upnasj">
      ...
      {toastMessage && (
        ...
        `}
          data-oid="c3lf_l:"
        >
          <div className="flex items-center" data-oid="jp13qi1">
            ...
            <span className="text-sm font-medium" data-oid="ubuyic5">
      ...
      {pendingMoves.size > 0 && (
        <div
          ...
          data-oid="7kp8z6o"
        >
          <div className="flex items-center" data-oid="uaivh8n">
            <svg
            ...
            <span className="text-sm font-medium" data-oid="d.9aw85">
