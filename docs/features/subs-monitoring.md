...
- **Leaders' children** (`/subs/leaders-children`, a tab beside Summary):
  ...
  or 2 has the same name as an adult leader. An adult leader is anyone whose
  ...
  extra spaces (`leadersChildrenModel.js`), and a member never matches
  themselves. Matching reads only the local cache, so it works offline and
  ...
  leader's sections, including "Adults") and Subs group. Subs group needs the
